...
ObservableList<Punition> liste_Punition; 
    ...
    private void handleButtonAction(ActionEvent event) {
        ...
        String cla = classes.getSelectionModel().getSelectedItem().getNom(); 
         ...
         if (cause.getText().length() != 0 && classes.getSelectionModel().getSelectedItem() != null && eleves.getSelectionModel().getSelectedItem() != null ) {
         if(ps.insertPunition(p)) {
 Alert alert = new Alert(AlertType.INFORMATION);
        alert.setTitle("Ajout Punition ");
   Notifications.create()
                     .title("punition ")
                     .text(" punitions est ajouter  avec succes  ").showWarning(); 
...
alert.showAndWait();    
         ...
         else if (cause.getText().length() == 0 || classes.getSelectionModel().getSelectedItem() == null || eleves.getSelectionModel().getSelectedItem() == null ) 
         {
         Alert alert = new Alert(AlertType.ERROR);
alert.setTitle("Error ");
alert.setHeaderText(" il faut saisir les champs ");
alert.setContentText("probleme");
